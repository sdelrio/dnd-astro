...
| 011 | Saving Throws card | archived | character-sheet, xml, fantasy-grounds, saving-throws, saves, xml-viewer, all-saves | `src/components/xml-viewer/XmlCard.astro`, `src/components/xml-viewer/xml-card-passives.test.ts`, `src/content/docs/guides/xml-card-test.mdx` | Large-mode all-saves card with two 3+3 tables of all six saving throws and gold proficiency dots |
...
| 014 | FG Effects Page Migration with Mermaid | archived | fantasy-grounds, mermaid, migration, starlight, diagrams, fg-effects | `package.json`, `astro.config.mjs`, `src/content/docs/fantasy-grounds/fg-effects.mdx`, `src/content/docs/index.mdx`, `docs/specs/014-fg-effects-migration/` | Faithful port of the golden-forest FG Effects page with its sequence diagram rendered via astro-mermaid |
| 015 | Half-proficiency skill marker | archived | character-sheet, xml, fantasy-grounds, skills, proficiency, xml-viewer, all-skills | `docs/specs/015-half-proficiency/SPEC.md`, `src/components/xml-viewer/skill-display.ts`, `src/components/xml-viewer/XmlCard.astro`, `src/content/docs/guides/xml-card-test.mdx` | Distinct half-filled gold dot for Fantasy Grounds `prof 3` half proficiency in the all-skills tables, plus a marker legend and a `skill-display` rank helper |
